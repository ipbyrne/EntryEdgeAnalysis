...
// Drawing Buffers
double dataBuffer[];
double dataBufferTwo[];
double dataBufferThree[];
  ...
  while(x<=1000)
   ...
   while(i>0)
    ...
    if(!longOp && !shortOp)// Confirm No Trades are Open
       {
       if(0>1) // Long Oppurtunity Started
          {
          startTime = i;
...
bool WriteFile()
  {
   int file_handle=FileOpen("Statistics"+"//"+"ERATIO-DC.txt",FILE_READ|FILE_WRITE|FILE_CSV|FILE_ANSI);
   if(file_handle!=INVALID_HANDLE)
     {
      PrintFormat("%s file is available for writing","ERATIO-DC.txt");
      ...
      PrintFormat("Data is written, %s file is closed","ERATIO-DC.txt");
      
      return(true);
     }
   else
     {
      PrintFormat("Failed to open %s file, Error code = %d","ERATIO-DC.txt",GetLastError());
      return(false);
     }
  }
